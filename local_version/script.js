...
// add-line button removed from DOM (auto-add behavior)
const addLineBtn = null;
...
function loadLists() {
  // rebuild lists from localStorage keys (simulate filesystem lists/*.json)
  ...
  // build displayName like the server: numeric filenames become "List N", else name without extension
  const built = files.map(f => {
    const m = f.match(/^(\d+)\.json$/);
    return { filename: f, displayName: m ? `List ${m[1]}` : f.replace(/\.json$/, '') };
  });
  // if no lists exist, seed with baseList using server-like filename 1.json (or next free)
  if (built.length === 0) {
    lists = [];
    return;
  }
...
function uniqueNameCandidate(orig, existingFiles) {
  // if orig doesn't exist return orig, else append -N
...
function createList() {
  // mimic server: pick next numeric filename and ensure unique
...
function importFile(file) {
  ...
  r.onload = function() {
    try {
      ...
      // mimic server POST: choose next numeric filename and unique name
      const existing = Object.keys(localStorage).filter(k => k.startsWith('fcc_list_')).map(k => k.replace(/^fcc_list_/, '')).filter(f => f.endsWith('.json') && !f.startsWith('_'));
...
function renameList(filename) {
  ...
  // fail if target exists (server returns 409)
...
function moveIndex(i, dir) {
  ...
  // mimic server reorder endpoint
  // reorderLists simply persists order array
}
...
function renderCombos() {
  combosEl.innerHTML = '';
  // When there are no lists, do not display combos (match Next.js behavior)
...
function addLineItem(name='', qty=1) {
  ...
  const addOption = (label, value, price) => {
    ...
    li.onclick = () => {
      if (value === '__custom__') {
        ...
        // auto-add new blank line if this is the last row
  ...
  // do not append popup inside wrapper to avoid clipping; show as portal appended to body
  let isOpen = false;
  button.onclick = (e) => {
    e.stopPropagation();
    if (!isOpen) {
...
saveComboBtn.onclick = () => {
  ...
  [...lineItemsEl.querySelectorAll('.line-item')].forEach(li => {
    // item may be custom-select wrapper, item-input, or button (wrapped)
...
// If there are no saved lists, start with nothing shown. If saved lists exist, select the first one and load it.
if (!Array.isArray(lists) || lists.length === 0) {
  combos = [];
  selectedList = null;
} else {
  // select first list and load its combos so the initial view matches selecting List 1
